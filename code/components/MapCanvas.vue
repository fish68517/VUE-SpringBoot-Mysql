...
export default {
 mounted(){this.mountedReady=true;if(this.data)this.update(this.data)},
 methods:{
  update(value,oldValue,owner,instance){if(!value)return;this.host=renderHost(this,owner,instance);this.data=value;if(!this.mountedReady)return;if(!value.active){this.release();return}if(!this.host)return;if(!this.canvas){this.init();requestAnimationFrame(()=>this.draw())}if(value.reset!==this.lastReset){this.zoom=1;this.offset=[0,0];this.lastReset=value.reset}if(value.zoom&&value.zoom!==this.lastZoom){this.zoom=value.zoom;this.lastZoom=value.zoom}this.draw()},
  init(){this.zoom=1;this.offset=[0,0];this.canvas=document.createElement('canvas');this.canvas.style.cssText='width:100%;height:100%;display:block;touch-action:none';this.host.appendChild(this.canvas);this.ctx=this.canvas.getContext('2d');this.resize=()=>this.draw();window.addEventListener('resize',this.resize);if(window.ResizeObserver){this.observer=new ResizeObserver(this.resize);this.observer.observe(this.host)};if(window.MutationObserver){this.removalObserver=new MutationObserver(()=>{if(!this.host.isConnected)this.release()});this.removalObserver.observe(document.body,{childList:true,subtree:true})}
  ...
  draw(){if(!this.canvas||!this.data)return;const rect=this.host.getBoundingClientRect();this.w=rect.width;this.h=rect.height;if(!this.w||!this.h)return;const ratio=Math.min(window.devicePixelRatio||1,2);this.canvas.width=this.w*ratio;this.canvas.height=this.h*ratio;const c=this.ctx;c.setTransform(ratio,0,0,ratio,0,0);this.scale=Math.min(this.w/1000,this.h/600)*.94;const d=this.data,dark=d.theme!=='light';c.fillStyle=dark?'#0b263b':'#ecf3f8';c.fillRect(0,0,this.w,this.h);
   ...
   c.font='10px sans-serif';c.textAlign='left';c.fillStyle=dark?'#708ea6':'#6d8293';c.fillText('供水示意图 · 非真实地理位置',16,this.h-16)
  },
