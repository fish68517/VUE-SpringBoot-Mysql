      ...
      var Ve = {
        ...
        default_tty_ops: {
          ...
          put_char: function put_char(e, t) {
            null === t || 10 === t ? ( e.output = []) : 0 != t && e.output.push(t);
          },
          flush: function flush(e) {
...
var url265 = "res/video40_265_moov_30s.mp4";
var url265_2 = "res/spreedmovie.mp4";
...
// start1btn.onclick = function () {
  var main = function main() {
    ...
    // rawParserObj = new MissileEngineDecoder.CRawParser();
		...
		let ws = new WebSocket("ws://10.1.54.254:8900");
		...
		ws.send(JSON.stringify(startParam))
		...
		if (typeof event.data == 'string') {
			...
			buffer = buffer.subarray(38, buffer.length);

			decoderMod.decodeNalu(buffer, ptsIdx);
			ptsIdx++;
  ...
  var t =0;
  function initWasm (canvasDom,params) {
    var canvas = document.querySelector(canvasDom ? canvasDom : '#canvas');
    var yuv = _renderYuv420p["default"].setupCanvas(canvas, {
      preserveDrawingBuffer: false
    });
    decoderMod = new _index["default"].CMissileDecoder(token, version);
    // window.decoderMods = decoderMod
  ...
  // console.log(window.decoderMods);
