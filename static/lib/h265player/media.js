...
(function (global, factory) {
  ...
  */
  ...
  var Frame = /*#__PURE__*/function () {
    ...
    function Frame() {
      var _this;
      // _this.nMediaFrameVersion = undefined;
      // _this.nEx = undefined;
      // _this.nIsKeyFrame = undefined;
      // _this.nTimetick = undefined;
      // _this.nIsAudio = undefined;
      // _this.StreamID = undefined;
      // _this.nSize = undefined;
      // _this.nOffset = undefined;
      // _this.nEncoder = undefined;
      // _this.nSPSLen = undefined;
      // _this.nPPSLen = undefined;
      // _this.nFps = undefined;
      // _this.nWidth = undefined;
      // _this.nHeight = undefined;
      // _this.nFrequency = undefined;
      // _this.nChannel = undefined;
      // _this.nAudioFormat = undefined;
      // _this.nSamples = undefined;
      // _this.Data = [];
      return _this;
    }
    ...
    _createClass(Frame, [{
      key: "getUint64",
      value: function getUint64(dataview, byteOffset, littleEndian) {
      ...
      key: "setBytes",
      value: function setBytes(data){
          var buffer = new Uint8Array(data);
            let view =  new DataView(data)

            this.nMediaFrameVersion  = view.getUint8(0, true);
            this.nEx = view.getUint8(1, true);
            this.nIsKeyFrame = view.getUint8(2, true);
            this.nTimetick = this.getUint64(view,3, true);;
            this.nIsAudio = view.getUint8(11, true);

            this.nSize  = view.getUint32(12, true);
            this.nOffset = view.getUint32(16, true);

            if(this.nMediaFrameVersion ==1 || this.nMediaFrameVersion ==2){
                this.nEncoder = view.getUint32(20, true);
                if(this.nIsAudio == 0){
                    this.nSPSLen = view.getUint16(24, true);
                    this.nPPSLen = view.getUint16(26, true);
                    this.nWidth = view.getUint32(28, true);
                    this.nHeight = view.getUint32(32, true);
                    this.nFps = view.getUint32(36, true);

                }
                if(this.nIsAudio == 1) {
                    this.nFrequency = view.getInt32(24,true);
                    this.nChannel = view.getInt32(28,true);
                    this.nAudioFormat =view.getUint16(32,true);
                    this.nSamples = view.getUint16(34,true);
                }
            }
            if(this.nMediaFrameVersion == 1 && this.nIsAudio == 0){
               // console.log("data fps == "+ this.nFps)
                this.nOffset =40;
            } else if(this.nMediaFrameVersion == 0 && this.nIsAudio == 0){
                this.nOffset =20;
            } else if(this.nMediaFrameVersion == 1 && this.nIsAudio == 1){
               // console.log("data nFrequency == "+ this.nFrequency)
                this.nOffset =36;

            } else if(this.nMediaFrameVersion == 0 && this.nIsAudio == 1){
                this.nOffset =20;
            } else {
              // console.log(this.nMediaFrameVersion);
              // console.log("type error");
            }

            this.Data = buffer.subarray(this.nOffset, buffer.length);
           // console.log(this.Data[0]+"-"+this.Data[1]+"-"+this.Data[2]+"-"+this.Data[3]+"-"+this.Data[4]);
        //    if(this.nMediaFrameVersion == 1 && this.nIsAudio == 0 && this.fps == 0)
        //    {
        //         let slices,left;
        //         [slices, left] = H264Parser.extractNALu(this.Data);

        //         if (slices.length > 0) {
        //                 for (let slice of slices) {
        //                     if((slice[0] & 0x1f) === 7){
        //                         var config = H264Parser.readSPS(new Uint8Array(sps));
        //                         this.nFps = config.fps;
        //                         break;
        //                     }
        //                 }
        //         }
  ...
  var H264Remuxer = /*#__PURE__*/function (_BaseRemuxer) {
    ...
    function H264Remuxer(timescale,fps) {
      var _this;
  ...
  var RemuxController = /*#__PURE__*/function (_Event) {
    ...
    function RemuxController(env,options) {
      var _this;
      ...
      _this.aac_timescale = options.aac_timescale;
      _this.h264_timescale = options.h264_timescale;
    ...
    _createClass(RemuxController, [{
      ...
      value: function addTrack(type) {
        if (type === 'video' || type === 'both') {
          this.tracks.video = new H264Remuxer(this.h264_timescale,this.fps);
          this.trackTypes.push('video');
        }

        if (type === 'audio' || type === 'both') {
          this.tracks.audio = new AACRemuxer(this.aac_timescale);
          ...
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            ...
            if (this.env == 'browser') {
              ...
              if(type === 'video'){
                _data= {
                    type: type,
                    payload: MP4.initSegment([track.mp4track], this.mediaDuration, this.h264_timescale),
                };
            }
            else if(type === 'audio'){
              _data = {
                    type: type,
                    payload: MP4.initSegment([track.mp4track], this.mediaDuration, this.aac_timescale),
                };
            }
      ...
      value: function isReady() {
        ...
        try {
          for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
            ...
            if (!this.tracks[type].readyToDecode) return false;
          }
        } catch (err) {
          _iterator4.e(err);
        } finally {
  ...
  var JMuxer = /*#__PURE__*/function (_Event) {
    ...
    function JMuxer(options) {
      ...
      _this.nlastTimeTick = 0;
      _this.nArrive = false;
      _this.currentFps = 0;

      _this.frame = new Frame();

      _this.currentSample = 0;
      _this.audio_remainingData = new Uint8Array();
      _this.vedio_remainingData = new Uint8Array();
    ...
    _createClass(JMuxer, [{
        ...
        value: function initData() {
          ...
          this.audio_remainingData = new Uint8Array();
          this.vedio_remainingData = new Uint8Array();
        ...
        value: function initMse() {
          ...
          this.remuxController = new RemuxController(this.env,this.options);
      ...
      value: function receiveData(data) {
        this.frame.setBytes(data)
        let buffer = this.frame.Data;

        if (this.options.mode == 'video') {
          if(this.currentFps == 0){
            if(this.frame.nFps == 0){
              this.currentFps =25;
              this.options.fps =25;
            }
            else{
              this.currentFps =this.frame.nFps;
              this.options.fps =this.frame.nFps;
            }
             
            console.log(this.frame.nEncoder);
            if(this.frame.nEncoder == 875967048){
              // document.getElementById("tess").innerText = '当前加载的是（H264）视频'
              //   document.getElementById("infoFPS").innerText = '当前加载的是（H264）视频，Fps：' + this.currentFps + '，' + Date()

              // document.getElementById("videosMSE").style.display= 'block'
              ...
              // document.getElementById("tess").innerText = '当前加载的是（H265）视频'

              document.getElementById("canvas").style.display= 'block'
              document.getElementById("videosMSE").style.display= 'none'
              window.isMSE = false;
            ...
            console.log("options.mode == video /currentFps == "+ this.currentFps+" /nEncoder == "+ this.frame.nEncoder+" /nWidth == "+this.frame.nWidth+" /nHeight == "+this.frame.nHeight)
          }

          if( this.videoType == 0){
            if(this.frame.nIsAudio == 0) {
              this.feed({
                  video: buffer,
              });
            }
            ...
            window.decoderMods.decodeNalu(buffer, ptsIdx);
            ptsIdx++;
          }
      }
      else if (this.options.mode == 'audio') {
          if(this.currentSample == 0){
              this.currentSample =this.frame.nFrequency;
              this.options.aac_timescale =this.frame.nFrequency;

              this.initMse();
              console.log("options.mode == audio currentSamples == "+ this.currentSample)
          }
          if(this.frame.nIsAudio == 1) {
              this.feed({
                  audio: buffer,
              });
          }
      }
      else if (this.options.mode == 'both') {
          if((this.currentSample == 0 || this.currentFps == 0) && this.frame.nIsAudio == 1){
              if(this.currentSample == 0){
                  this.currentSample =this.frame.nFrequency;
                  this.options.aac_timescale =this.frame.nFrequency;
              }
              this.audio_remainingData = appendByteArray(this.audio_remainingData,buffer);
          }
          if((this.currentFps == 0 || this.currentSample == 0) && this.frame.nIsAudio == 0){
              if(this.currentFps == 0){
                  this.currentFps =this.frame.nFps;
                  this.options.fps =this.frame.nFps;
              }
              this.vedio_remainingData = appendByteArray(this.vedio_remainingData,buffer);
          }
          if(this.currentFps != 0 && this.currentSample != 0 && this.isInit != 1){
              ...
              if(this.isInit ==0){
                  ...
                  this.initMse();
                  this.isInit =1;
                  console.log("options.mode == both currentSamples == "+ this.currentSample+"  currentFps == "+ this.currentFps)
              }
              if(this.isInit == 2){
                  if(this.frame.nIsAudio == 0 && this.frame.nIsKeyFrame ==1){
                      this.reset1()
                      this.isInit =1;
                      console.log("mse is reseted")
                  }
              }
              if(this.frame.nIsAudio ==0 ){
                  this.feed({
                      video: this.vedio_remainingData,
                  });
              }
              if(this.frame.nIsAudio ==1 ){
                  this.feed({
                      audio: this.audio_remainingData,
                  });
              }
             // this.isInit =1;
              this.audio_remainingData = new Uint8Array();
          ...
          if(this.currentFps != 0 && this.currentSample != 0 && this.isInit){
              if(this.frame.nIsAudio ==0 ){
                  this.feed({
                      video: buffer,
                  });
      ...
      value: function getVideoFrames(nalus, duration) {
        ...
        frames.map(function (frame) {
          frame.duration = 90000 / _this2.currentFps;
      ...
      value: function destroy() {
        ...
        if(this.videoType ==1){
          window.decoderMods.release();
          window.decoderMods = null;
        }
      ...
      value: function reset1() {
        ...
        if (this.bufferControllers) {
            for (let type in this.bufferControllers) {
                this.bufferControllers[type].reset();
            }
            //this.bufferControllers = null;
        }
        this.initData()
      ...
      value: function startInterval() {
        ...
        }, _this3.options.flushingTime || 1000);
      }
    }, {
      key: "stopInterval",
      ...
      key: "readDelay",
      value: function readDelay() {
        if (this.node.buffered && this.node.buffered.length > 0 && !this.node.seeking) {
          const end = this.node.buffered.end(0);
          var dif = end - this.node.currentTime;
          if(dif > 5) {
              // console.log(new Date() +"------dif >= 5   dif == "+dif);
              this.node.currentTime = end - 1;
          }
          if(dif >= 1) {
              // console.log(new Date() +"------dif >= 1 && dif <=2 dif == "+dif);
              if(this.node.playbackRate!=1.15)
                 this.node.playbackRate = 1.15
          }
          else if(dif < 1 && dif > 0.7) {
          //   console.log(new Date() +"dif < 1 && dif > 0.7 dif == "+dif);
              if(this.node.playbackRate!=1.1)
              this.node.playbackRate = 1.1
          }
          else if(dif <= 0.7 && dif>= 0.3) {
              // console.log(new Date() +"dif <= 0.7 && dif >= 0.3 dif == "+dif);
              if(this.node.playbackRate!=1)
              this.node.playbackRate = 1
          }
          else if(dif <0.3) {
              // console.log(new Date() +"dif < 0.3 && dif >= 0.3 dif == "+dif);
              if(this.node.playbackRate!=0.7)
              this.node.playbackRate = 0.7
          }
          else {
              // console.log(new Date() +"else dif == "+dif);
      ...
      value: function onBuffer(data) {
        if (this.env == 'browser') {
          if(this.bufferControllers == undefined){
            console.log("this.bufferControllers == undefined");
            this.createBuffer() ;
          }
      ...
      key: "wsReady",
      value: function wsReady(ip, agree, token, select_id) {
        let that = this

        let datas = {
          SingalType:1,
          SignalName:"WsLiveStartRequest",
          Timeout:30,
          __jsonModel:{
            Token: token,
            LiveType: select_id,
        ...
        this.ws.onmessage = (event) => {
          if (typeof event.data == 'string') {
            ...
            that.receiveData(event.data)
            that.statAction = true;
          }
        };
        this.ws.onerror = (err) => {
          alert('连接不成功，请检查ip链接')
        ...
        this.ws.addEventListener('open', function (event) {
          that.ws.send(JSON.stringify(datas))
        })
      ...
      key: "wsClose",
      value: function wsClose(ip, agree, token,select_id) {
        let closeDatas = {
          SingalType:1,
          SignalName:"WsLiveStopRequest",
          Timeout:30,
          __jsonModel:{
            Token: token,
            LiveType:select_id,
        ...
        if (this.ws) {
          ...
          this.ws.send(JSON.stringify(closeDatas))
          this.ws.close()
          this.statAction = false;
        }else{
          return
        }
      }
...
var tenthFromBottom = null;
var ninthFromBottom = null;
var eighthFromBottom = null;
var seventhFromBottom = null;
var sixthFromBottom = null;
var fifthFromBottom = null;
var fourthFromBottom = null;
var thirdFromBottom = null;
var lastFrame = null;
...
function wsReady(ip, agree, token, select_id, domId, canvasId) {
    let datas = {
      SingalType:1,
      SignalName:"WsLiveStartRequest",
      Timeout:30,
    ...
    ws.onmessage = (event) => {
      if (typeof event.data == 'string') {
        ...
        // 在这需要判断265或者264 进行数据分类展示
        point(event.data,select_id, domId, canvasId)
...
// WebSocket停止
function wsClose(ip, agree, token,select_id) {
  let closeDatas = {
    SingalType:1,
    SignalName:"WsLiveStopRequest",
    Timeout:30,
  ...
  if (ws) {
    if(frame.nEncoder == 875967048 || frame.nEncoder == 1598243137){
      jmuxer.destroy();
    }
    //stat = false;
    isVideoType = false;
    ws.send(JSON.stringify(closeDatas))
    ws.close()
    statAction = false;
    firstOne = true;
    // canvas.style.display = 'none';
    // document.getElementById("infoFPS").innerText = ''
    sample = [];
    count = 0;
...
// 数据分源
function setframe(data,isKeyFrame){
  if(maxCount >= count){
...
function point(data,select_idVal, domId, canvasId){
  // if (jmuxer?.statAction) {
  //   alert('请先停止上一个视频')
  //   return
  // }
  // let select_idVal = document.getElementById(select_id).value
  // if (select_idVal == 1) {
  //   jmuxerData.mode = 'both'
  // }else if (select_idVal == 2) {
  //   jmuxerData.mode = 'video'
  // }
  // console.log(select_idVal);

  // jmuxer.wsReady(ipVal,agreeVal,tokenVal,select_idVal)

  frame.setBytes(data)
  let buffer = this.frame.Data;
  let nIsKeyFrame = this.frame.nIsKeyFrame;

  if(frame.nEncoder == 875967048){
    if (firstOne) {
      firstOne = false
      let jmuxerData = {
        node: domId,
      ...
      jmuxer = new JMuxer(jmuxerData);

    }

    // document.getElementById("infoFPS").innerText = '当前加载的是（H264）编码视频，分辨率：' + frame.nWidth + "*" + frame.nHeight
    document.getElementById(domId).style.display= 'block'
    document.getElementById(canvasId).style.display= 'none'
    window.isMSE = true;

    jmuxer.receiveData(data)
    jmuxer.statAction = true;

  } 
  else if (frame.nEncoder == 892744264){
    var decoderMod;
    if (firstOne) {
      firstOne = false;
      window.decoderMod = new window.initWasm(canvasId)
    //  var decoderMod = initW.init()
    }
    

    // document.getElementById("infoFPS").innerText = '当前加载的是（H265）编码视频，分辨率：' + frame.nWidth + "*" + frame.nHeight
    ...
    // var temp = setframe(buffer,nIsKeyFrame);
    // if(temp != null){
    //   window.decoderMod.decodeNalu(temp.data, ptsIdx);
    //    ptsIdx++;
    // }


      window.decoderMod.decodeNalu(buffer, ptsIdx);
      ptsIdx++;


  }
  else if (frame.nEncoder == 877677894){
    var decoderMod;
    if (firstOne) {
      firstOne = false;
      window.decoderMod = new window.initWasm(canvasId)
  ...
  else if(!firstOne && select_idVal == 1){
      jmuxer.receiveData(data)
    }
  }

function play(ip, agree, token ,select_id, domId, canvasId) {
  ...
  // let ipVal = document.getElementById(ip).value
  // let agreeVal = document.getElementById(agree).value
  // let select_idVal = document.getElementById(select_id).value

  // wsReady(ipVal,agreeVal,tokenVal,select_idVal)
  wsReady(ip,agree,token,select_id, domId, canvasId)
  // 以上是新改
  // if (jmuxer?.statAction) {
  //   alert('请先停止上一个视频')
  //   return
...
function stop(ip,agree,token,select_id) {
  ...
  // jmuxer.wsClose(ipVal,agreeVal,tokenVal,select_idVal)

  wsClose(ip,agree,token,select_id)
