  ...
  <div class="application-review-container">
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button 
              ...
              :disabled="row.hospitalStatus !== 'PENDING' || row.schoolStatus !== 'APPROVED'">
              审批
            </el-button>
            <el-button 
              type="info" 
    ...
    <el-dialog v-model="reviewDialogVisible" title="申请审批" width="500px">
      <div v-if="selectedApplication" class="review-form">
        ...
        <div class="form-group">
          <label>学校审批意见：</label>
          <p class="reason-text">{{ selectedApplication.schoolOpinion || '暂无' }}</p>
        </div>
        <div class="form-group">
          <label>审批意见：</label>
          <el-input 
...
import { getHospitalApplications, hospitalReviewApplication } from '@/api/hospitalAdmin'
...
const submitReview = async () => {
  try {
    await hospitalReviewApplication(selectedApplication.value.id, {
