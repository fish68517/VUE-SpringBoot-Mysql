  ...
  setup() {
    ...
    const handleLogin = async () => {
      ...
      try {
        ...
        //  打印 response
        console.log("登录成：",JSON.stringify(response))

        // 保存认证信息
        if (response && response.data) {
          const resData = response.data
          
          // 1. 根据后端返回的实际结构，手动构造 user 对象
          // authService 期望的 user 对象中包含 id 字段，我们将 userId 映射给它
          const user = {
            id: resData.userId,
          ...
          // 2. 检查后端是否返回了 token。
          // 注意：你的日志中没有 token 字段。如果后端使用 Session 而不需要 Token，
          // 为了让前端 authService.isAuthenticated() 不被拦截，这里需要给一个占位符 Token。
          const token = resData.token || 'mock-token-to-pass-auth'
          
          // 3. 使用后端返回的角色（如果没有则使用用户在前端选择的角色）
          const role = resData.role || selectedRole.value
