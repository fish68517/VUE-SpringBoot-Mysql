    ...
    <div v-else class="form-card">
      ...
      >
        ...
        <template v-if="formData.gender === 'MALE'">
          <!-- 健康状态 -->
          <el-form-item :label="$t('genderHealth.prostateHealth') || '健康状态'" prop="prostateHealth">
            <el-input
...
export default {
  ...
  setup() {
    ...
    const formRules = {
      ...
      prostateHealth: [
        { required: false, message: '健康状态为可选项', trigger: 'blur' }
      ],
      sexualFunction: [
    ...
    const fetchRecentRecords = async () => {
      try {
        ...
        if (response && response.data) {
          ...
          recentRecords.value = response.data.slice(0, 1000)
        }
      } catch (error) {
      ...
      try {
        ...
        const submitRequest = {
          ...
          dietRecord: formData.gender === 'FEMALE' 
            ...
            : `健康状态:${formData.prostateHealth},性功能:${formData.sexualFunction}`,
