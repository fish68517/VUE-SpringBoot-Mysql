<template>
  <div class="health-data-input-container">
    ...
    <div class="page-header">
      <h2>{{ $t('healthData.submitData') || '提交健康数据' }}</h2>
    </div>
      ...
      >
        ...
        <el-form-item :label="$t('healthData.dietRecord') || '饮食记录'" prop="dietRecord">
          <el-input
            ...
            type="textarea"
            :rows="3"
            :placeholder="$t('healthData.dietRecord') || '请记录今天的饮食情况（可选）'"
            clearable
            maxlength="500"
            show-word-limit
          />
        ...
        <el-form-item :label="$t('healthData.exerciseRecord') || '运动记录'" prop="exerciseRecord">
          <el-input
            v-model="formData.exerciseRecord"
            type="textarea"
            :rows="3"
            :placeholder="$t('healthData.exerciseRecord') || '请记录今天的运动情况（可选）'"
            clearable
            maxlength="500"
            show-word-limit
          />
    ...
    <div v-if="!isLoading && recentData.length > 0" class="recent-data-card">
      <h3>{{ $t('healthData.recentData') || '最近提交的数据' }}</h3>
...
export default {
  ...
  setup() {
    ...
    const formRules = {
      ...
      dietRecord: [
        { required: false, message: '饮食记录为可选项', trigger: 'blur' }
      ],
      exerciseRecord: [
        { required: false, message: '运动记录为可选项', trigger: 'blur' }
      ],
      recordedAt: [
        { required: false, message: '记录时间为可选项', trigger: 'blur' }
      ]
    }
    ...
    const fetchRecentData = async () => {
      try {
        ...
        if (response && response.data) {
          // 只显示最近5条数据
          recentData.value = response.data.slice(0, 100000)
        }
      } catch (error) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (response && response.data) {
          successMessage.value = response.message || '健康数据已提交'
        ...
        if (error.response && error.response.data) {
          errorMessage.value = error.response.data.message || '提交健康数据失败'
        } else if (error.message) {
          errorMessage.value = error.message
        } else {
          errorMessage.value = '提交健康数据失败'
        }
      } finally {
