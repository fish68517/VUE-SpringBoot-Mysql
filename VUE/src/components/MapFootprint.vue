...
onMounted(async () => {
  try {
    ...
    ElMessage.error('地图初始化失败，请检查高德地图 Key 配置。')
  }
})
...
const handleSubmitFootprint = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.message || '添加足迹失败')
  } finally {
