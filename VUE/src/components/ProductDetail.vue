...
const handleAddToCart = async () => {
  ...
  // 【核心修改点】调用刚才 store 里写的 API 方法，真正存入数据库
  // 传入商品的 ID 和 选择的数量
  console.log('handleAddToCart', product.value.id, quantity.value)
...
<style scoped>
/* 样式保持不变，略... */
.product-detail-container { background: white; border-radius: 8px; padding: 20px; }
.detail-header { margin-bottom: 20px; }
.detail-content { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; }
.image-section { display: flex; flex-direction: column; gap: 15px; }
.main-image { width: 100%; aspect-ratio: 1; background: #f5f5f5; border-radius: 8px; overflow: hidden; }
.main-image img { width: 100%; height: 100%; object-fit: cover; }
.thumbnail-gallery { display: flex; gap: 10px; overflow-x: auto; }
.thumbnail { width: 80px; height: 80px; border: 2px solid transparent; border-radius: 4px; overflow: hidden; cursor: pointer; transition: all 0.3s ease; }
...
.product-title { font-size: 28px; font-weight: bold; color: #333; margin: 0; }
.price-section { display: flex; align-items: center; gap: 15px; }
.current-price { font-size: 32px; font-weight: bold; color: #e74c3c; }
.original-price { font-size: 18px; color: #999; text-decoration: line-through; }
.discount { background: #e74c3c; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: bold; }
.stock-section { font-size: 14px; }
.in-stock { color: #67c23a; font-weight: bold; }
.out-of-stock { color: #f56c6c; font-weight: bold; }
.description-section { padding: 15px; background: #f9f9f9; border-radius: 8px; }
.description-section h3 { margin: 0 0 10px 0; font-size: 14px; font-weight: bold; color: #333; }
.description-section p { margin: 0; font-size: 14px; color: #666; line-height: 1.6; }
.specs-section { padding: 15px; background: #f9f9f9; border-radius: 8px; }
.specs-section h3 { margin: 0 0 15px 0; font-size: 14px; font-weight: bold; color: #333; }
.spec-group { margin-bottom: 15px; }
.spec-group label { display: block; font-weight: bold; color: #333; margin-bottom: 8px; font-size: 14px; }
.spec-options { display: flex; gap: 10px; flex-wrap: wrap; }
.quantity-section { display: flex; align-items: center; gap: 15px; }
.quantity-section label { font-weight: bold; color: #333; white-space: nowrap; }
.action-buttons { display: flex; gap: 15px; }
.action-buttons :deep(.el-button) { flex: 1; }
.loading-state { padding: 40px 20px; }
@media (max-width: 768px) { .detail-content { grid-template-columns: 1fr; gap: 20px; } .product-title { font-size: 20px; } .current-price { font-size: 24px; } .action-buttons { flex-direction: column; } }
</style>