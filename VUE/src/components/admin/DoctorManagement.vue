  ...
  setup() {
    ...
    const fetchDoctors = async () => {
      try {
        ...
        // 1. 使用 Promise.all 并发获取医师列表和所有的用户列表
        ...
        if (doctorsResponse && doctorsResponse.data) {
          ...
          const usersList = (usersResponse && usersResponse.data) ? usersResponse.data : []

          // 2. 遍历医师列表，将对应的 user 信息挂载到 doctor 对象上
          doctors.value = rawDoctors.map(doctor => {
            // 根据 doctor.userId 在用户列表中寻找匹配的用户
            const matchedUser = usersList.find(u => u.id === doctor.userId) || {
              username: '未知(或已删)',
              name: '-'
            }
            
            // 3. 组装并返回带 user 对象的完整数据，适配页面上的 prop="user.username"
            return {
              ...doctor,
              user: matchedUser
            }
        ...
        errorMessage.value = error.response?.data?.message || '获取医师或关联用户数据失败'
      } finally {
