  ...
  <AdminLayout>
          ...
          <div v-for="activity in activities" :key="activity.id" class="activity-item">
              ...
              <div class="activity-info">
                ...
                <p class="activity-meta">
                  <span class="meta-item">
                    <strong>组织者：</strong> {{ activity.organizer?.nickname || '未知' }}
                  </span>
                  <span class="meta-item">
                    <strong>类型：</strong> {{ activity.type || '未分类' }}
                  </span>
                  <span class="meta-item">
                    <strong>提交时间：</strong> {{ formatDate(activity.createdAt) }}
                  </span>
                </p>
              </div>
            ...
            <div class="activity-details">
              ...
              <div class="detail-row">
                ...
                <span class="detail-value">{{ activity.location || '未指定' }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">参与人数限制：</span>
                <span class="detail-value">{{ activity.maxParticipants || '无限制' }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">报名截止时间：</span>
                <span class="detail-value">{{ formatDateTime(activity.registrationDeadline) || '未设置' }}</span>
              </div>
              <div v-if="activity.enableCrowdfunding" class="detail-row">
                <span class="detail-label">众筹目标金额：</span>
                <span class="detail-value">¥{{ activity.crowdfundingTarget }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">活动描述：</span>
                <span class="detail-value description">{{ activity.description || '无描述' }}</span>
              </div>
            </div>
            ...
            <div class="audit-actions">
              <button
                class="btn-approve"
                @click="openApproveDialog(activity)"
                :disabled="submittingId === activity.id"
              >
                {{ submittingId === activity.id ? '处理中...' : '通过' }}
              </button>
              <button
                class="btn-reject"
                @click="openRejectDialog(activity)"
                :disabled="submittingId === activity.id"
              >
                {{ submittingId === activity.id ? '处理中...' : '驳回' }}
              </button>
            </div>
          </div>
        </div>
    ...
    <div v-if="showApproveDialog" class="dialog-overlay" @click="closeApproveDialog">
      <div class="dialog-content" @click.stop>
        ...
        <div class="dialog-body">
          <p>确定要通过审核活动 <strong>{{ selectedActivity?.title }}</strong> 吗？</p>
          <p class="info-text">通过后，活动将发布至首页活动列表。</p>
        </div>
    ...
    <div v-if="showRejectDialog" class="dialog-overlay" @click="closeRejectDialog">
      <div class="dialog-content" @click.stop>
        ...
        <div class="dialog-body">
          <p>确定要驳回活动 <strong>{{ selectedActivity?.title }}</strong> 吗？</p>
...
/**
 * Load pending audit activities
 */
const loadActivities = async (page = 0) => {
  loading.value = true
  try {
    const response = await http.get('/audit/activities', {
      params: {
        page,
        size: pageSize.value
      }
...
const submitApprove = async () => {
  ...
  submittingId.value = selectedActivity.value.id

  try {
    const response = await http.put(`/audit/activities/${selectedActivity.value.id}`, {
      approved: true,
      reason: null
    })

    // Remove approved activity from list
...
const submitReject = async () => {
  if (!selectedActivity.value || !rejectReason.value.trim()) {
  ...
  try {
    ...
    // Remove rejected activity from list
    activities.value = activities.value.filter(a => a.id !== selectedActivity.value.id)
    totalCount.value--
