...
<template>
  <GlobalSetting :is-use-custom="true" :tabData="tabData">
    <template #other>
      <div class="flood-config-wrap">
        <div class="flood-config-item">
          <span class="flood-config-label">三联屏模式</span>
          <n-checkbox v-model:checked="tripleScreen" />
        </div>
        <div class="flood-config-item">
          <span class="flood-config-label">洪峰流速</span>
          <n-input-number
            v-model:value="floodPeakVelocity"
            :step="0.1"
            :min="0.1"
            size="small"
            style="width: 120px"
          />
          <span class="flood-config-unit">米/秒</span>
        </div>
      </div>
    </template>
  </GlobalSetting>
...
const { HelpCircleSharpIcon, ArchiveIcon } = icon.ionicons5
...
// 洪峰流速配置
const floodPeakVelocity = ref(props.targetData?.floodPeakVelocity ?? 6.2)
watch(floodPeakVelocity, (val) => {
  if (props.targetData) {
    props.targetData.floodPeakVelocity = val
  }
})

// 三联屏模式配置
const tripleScreen = ref(props.optionData?.tripleScreen !== false)
watch(tripleScreen, (val) => {
  if (props.optionData) {
    props.optionData.tripleScreen = val
  }
})
