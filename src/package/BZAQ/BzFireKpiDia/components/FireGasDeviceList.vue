<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">燃气设备详情</div>
...
const columns = [
  ...
  { title: '区域名称', key: 'areaName', width: 140 },
  { title: '设备编号', key: 'deviceNo', width: 180 },
  { title: '设备名称', key: 'deviceName', minWidth: 220 },
  { title: '设备类型', key: 'deviceType', width: 140 },
  { title: '安装地址', key: 'address', minWidth: 260 },
  { title: '安装时间', key: 'installationTime', width: 170 },
  { title: '所属公司', key: 'company', minWidth: 200 },
  {
    title: '状态',
    key: 'status',
    width: 100,
    render(row) {
      return row?.status === 1 || row?.status === '1' ? '正常' : row?.status || '-'
    }
  }
]
...
const getGasDeviceList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/location/gas/detail`, getQueryParams())
    const pageData = getResponsePage(res?.data)
    tableData.value = pageData.records
    total.value = pageData.total
  } catch (err) {
    console.error('获取燃气设备列表失败:', err)
    tableData.value = []
    total.value = 0
  } finally {
    loading.value = false
