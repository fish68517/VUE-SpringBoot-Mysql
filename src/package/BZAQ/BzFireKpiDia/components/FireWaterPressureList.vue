<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">水压监测详情</div>
...
const columns = [
  ...
  { title: '当日计数', key: 'dayCount', width: 100 },
  { title: '90天计数', key: 'ninetyCount', width: 100 },
  { title: '上报时间', key: 'reportTime', width: 170 },
  { title: '报警状态', key: 'alarmStatus', width: 110 },
  { title: '最小压力', key: 'minPressure', width: 100 },
  { title: '最大压力', key: 'maxPressure', width: 100 },
  { title: '监测值', key: 'monitorValue', width: 100 },
  { title: '监测值单位', key: 'monitorValueUnit', width: 110 }
]
...
const getQueryParams = () => ({
  ...
  buildName: props.buildName || undefined
})
...
const getWaterPressureList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/location/water/detail`, getQueryParams())
    const pageData = getResponsePage(res?.data)
    tableData.value = pageData.records
    total.value = pageData.total
  } catch (err) {
    console.error('获取水压监测列表失败:', err)
    tableData.value = []
    total.value = 0
  } finally {
    loading.value = false
