<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">高层建筑详情</div>
...
const columns = [
  ...
  { title: '街道', key: 'xzjd', width: 120 },
  { title: '建筑名称', key: 'gcjzmc', minWidth: 220 },
  { title: '建筑类型', key: 'jzdx', width: 120 },
  { title: '详细地址', key: 'xxdz', minWidth: 260 },
  { title: '管理单位', key: 'glzthztjzd', minWidth: 220 },
  { title: '重点对象', key: 'keyObjects', width: 120 },
  { title: '建筑年代', key: 'jcnd', width: 100 },
  { title: '高度', key: 'gd', width: 90 },
  { title: '未销案隐患数', key: 'unresolvedCases', width: 120 },
  { title: '已销案隐患数', key: 'closedCases', width: 120 },
  { title: '五色预警扣分数', key: 'fiveColourScoreCount', width: 140 },
  { title: '风险等级', key: 'fxdj', width: 130 },
  { title: '消防安全管理人', key: 'xfaqglr', width: 140 },
  { title: '联系方式', key: 'lxfs', width: 140 },
  { title: '网格员', key: 'wgy', width: 120 }
]
...
const getHighBuildingList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/location/build/detail`, getQueryParams())
    const pageData = getResponsePage(res?.data)
    tableData.value = pageData.records
    total.value = pageData.total
  } catch (err) {
    console.error('获取高层建筑列表失败:', err)
    tableData.value = []
    total.value = 0
  } finally {
    loading.value = false
