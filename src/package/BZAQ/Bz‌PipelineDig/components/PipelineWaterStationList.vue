...
const columns = [
  ...
  { title: '安装地址', key: 'installAddress', width: 330 },
  { title: '设备厂商', key: 'vendor', width: 160 },
...
async function fetchRows() {
  try {
    const res = await axios.post(`${BASE_URL}/api/boot/system/pipeline/waterQualityStation`, {
      index: 1,
      size: 1000,
    ...
    console.error('获取水质监测站列表失败', error)
    rows.value = []
  }
}
