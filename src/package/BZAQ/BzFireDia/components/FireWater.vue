<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">消防水管监控</div>
    ...
    <div class="filter-bar">
      <div class="filter-item">
        ...
        <n-select v-model:value="selectedStreetName" class="filter-select" :options="streetOptions" :theme-overrides="selectThemeOverrides" size="small" />
      </div>
...
const columns = [
  ...
  { title: '乡镇街道', key: 'streetName', width: 100, ellipsis: { tooltip: true } },
  { title: '设备编号', key: 'deviceNum', width: 160, ellipsis: { tooltip: true } },
  { title: '设备名称', key: 'deviceName', minWidth: 220, ellipsis: { tooltip: true } },
  { title: '设备分类', key: 'deviceType', width: 110, ellipsis: { tooltip: true } },
  { title: '安装地址', key: 'address', minWidth: 220, ellipsis: { tooltip: true } },
  { title: '建筑名称', key: 'buildName', width: 150, ellipsis: { tooltip: true } },
  { title: '上次预警时间', key: 'warningTime', width: 160, ellipsis: { tooltip: true } },
  { title: '最小压力', key: 'minPressure', width: 80, ellipsis: { tooltip: true } },
  { title: '最大压力', key: 'maxPressure', width: 80, ellipsis: { tooltip: true } },
  { title: '监测值', key: 'monitorValue', width: 90, ellipsis: { tooltip: true }, render: (row) => `${row.monitorValue || '-'}${row.monitorValueUnit || ''}` },
  { title: '处理次数', key: 'handleNum', width: 80, ellipsis: { tooltip: true } }
]

// 获取消防水监测设备预警列表
const getFireWaterWarningList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/fireWater/warning`, {
    ...
    if (res.data.code === '000000') {
      ...
      console.error('获取消防水监测设备预警失败：' + res.data.message)
    }
  } catch (err) {
    console.error('请求消防水监测预警接口失败', err)
  } finally {
    loading.value = false
  }
}
...
watch([selectedStreetName, timeRange], () => {
  currentPage.value = 1
  getFireWaterWarningList()
...
:deep(.fire-table) {
  ...
  .n-data-table-base-table-body .n-scrollbar-container::-webkit-scrollbar {
    height: 8px;
  }

  .n-data-table-base-table-body::-webkit-scrollbar-thumb,
  .n-data-table-base-table-body .n-scrollbar-container::-webkit-scrollbar-thumb {
    background: rgba(83, 174, 255, 0.45);
    border-radius: 4px;
  }

  .n-data-table-base-table-body::-webkit-scrollbar-track,
  .n-data-table-base-table-body .n-scrollbar-container::-webkit-scrollbar-track {
    background: rgba(16, 64, 126, 0.28);
  }
