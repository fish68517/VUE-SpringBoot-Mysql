<template>
  <div class="fire-maintenance-missing-modal">
    <div class="modal-header">
      <div class="header-title">近期未开展消防设施维保</div>
    ...
    <div class="filter-bar">
      ...
      <div class="filter-item">
        <span class="filter-label">近3月不到位</span>
        <n-select v-model:value="selectedRecentLack" class="filter-select" :options="recentLackOptions" :theme-overrides="selectThemeOverrides" size="small" />
      </div>
...
const columns = [
  ...
  { title: '乡镇街道', key: 'street', width: 100, ellipsis: { tooltip: true } },
  { title: '所属社区', key: 'community', width: 100, ellipsis: { tooltip: true } },
  { title: '高层建筑名称', key: 'buildingName', minWidth: 150, ellipsis: { tooltip: true } },
  { title: '地址', key: 'buildingAddress', minWidth: 260, ellipsis: { tooltip: true } },
  { title: '建筑定性', key: 'buildingProperties', width: 120, ellipsis: { tooltip: true } },
  { title: '管理单位', key: 'managementUnit', minWidth: 260, ellipsis: { tooltip: true } },
  { title: '消防安全管理人', key: 'fireSafetyManager', width: 100, ellipsis: { tooltip: true } },
  { title: '维保不到位', key: 'inadequate', width: 120, ellipsis: { tooltip: true } },
  {
    title: '操作',
    key: 'action',
    width: 80,
    ellipsis: { tooltip: true },
    render: () => h('span', { style: { color: '#53aeff', cursor: 'pointer', textDecoration: 'underline' } }, '查看记录')
  }
]
...
const getMaintenanceList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/fir-equ`, {
      pageNumber: currentPage.value,
      pageSize: pageSize,
      street: selectedStreet.value || undefined,
      inadequate: selectedRecentLack.value || undefined,
    ...
    if (res.data.code === '000000') {
      ...
      NMessage.error('获取维保列表失败：' + res.data.message)
    }
  } catch (err) {
    console.error('请求维保接口失败', err)
    NMessage.error('接口请求失败，请检查网络连接')
  } finally {
    loading.value = false
  }
}
...
watch([selectedStreet, selectedRecentLack, timeRange], () => {
  currentPage.value = 1
  getMaintenanceList()
...
.fire-maintenance-missing-modal {
