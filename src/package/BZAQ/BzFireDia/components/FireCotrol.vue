<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">消控室监测</div>
...
const columns = [
  ...
  { title: '告警类型', key: 'warningType', width: 120, ellipsis: { tooltip: true } },
  { title: '地址', key: 'address', minWidth: 220, ellipsis: { tooltip: true } },
  { title: '上次预警时间', key: 'warningTime', width: 160, ellipsis: { tooltip: true } },
  { title: '处理状态', key: 'handleStatus', width: 100, ellipsis: { tooltip: true } },
  { title: '处理次数', key: 'handleNum', width: 80, ellipsis: { tooltip: true } },
  // { title: '今日次数', key: 'todayCs', width: 80 },
  { title: '总次数', key: 'cs', width: 80, ellipsis: { tooltip: true } }
]

// 获取监控室预警列表
const getRoomWarningList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/room/warning`, {
    ...
    if (res.data.code === '000000') {
      ...
      console.error('获取监控室预警失败：' + res.data.message)
    }
  } catch (err) {
    console.error('请求监控室预警接口失败', err)
  } finally {
    loading.value = false
  }
}
