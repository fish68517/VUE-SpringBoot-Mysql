<template>
  <div class="fire-passage-modal">
    <div class="modal-header">
      <div class="header-title">公共重点区域化粪池监管</div>
...
const columns = [
  ...
  { title: '告警ID', key: 'id', width: 120, ellipsis: { tooltip: true } },
  { title: '设备编码', key: 'dvcode', width: 160, ellipsis: { tooltip: true } },
  { title: '设备名称', key: 'dvname', minWidth: 220, ellipsis: { tooltip: true } },
  { title: '型号名称', key: 'modelname', width: 150, ellipsis: { tooltip: true } },
  { title: '安装地址', key: 'mountAddress', minWidth: 220, ellipsis: { tooltip: true } },
  { title: '区域名称', key: 'areaName', width: 120, ellipsis: { tooltip: true } },
  { title: '上次预警时间', key: 'warningTime', width: 160, ellipsis: { tooltip: true } },
  { title: '告警状态', key: 'warningStatus', width: 100, ellipsis: { tooltip: true } },
  { title: '处理次数', key: 'handleNum', width: 80, ellipsis: { tooltip: true } }
]

// 获取化粪池预警列表
const getSepticWarningList = async () => {
  loading.value = true
  try {
    const res = await axios.post(`${BASE_URL}/list/septic/warning`, {
    ...
    if (res.data.code === '000000') {
      ...
      console.error('获取化粪池预警列表失败：' + res.data.message)
    }
  } catch (err) {
    console.error('请求化粪池预警接口失败', err)
  } finally {
    loading.value = false
  }
}
