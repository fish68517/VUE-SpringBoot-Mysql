<template>
  <div class="fire-rectify-illegal-modal">
    <div class="modal-header">
      <div class="header-title">责令立即整改的消防安全违法行为</div>
    ...
    <div class="modal-content">
      <div class="list-item" v-for="(item, index) in listData" :key="index">
        ...
        <span class="item-count">
          <span class="count-left">{{ item.left }}</span>
          <span class="count-divider">/</span>
          <span class="count-right">{{ item.right }}</span>
        </span>
      </div>
    </div>
  </div>
...
const listData = ref([
  { text: '消防设施、器材/消防安全标志的配置、设置不符合标准;', left: '11', right: '11' },
  { text: '消防设施、器材/消防安全标志未保持完好有效;', left: '11', right: '11' },
  { text: '损坏/挪用消防设施、器材;', left: '11', right: '11' },
  { text: '擅自拆除/停用消防设施、器材;', left: '11', right: '11' }
])
...
.list-item {
  ...
  .item-count {
    ...
    .count-left {
      background: linear-gradient(to bottom, #ffd959, #ffa023);
    ...
    .count-right {
      background: linear-gradient(to bottom, #53aeff, #2af5ff);
      -webkit-background-clip: text;
      background-clip: text;
      color: transparent;
    }
