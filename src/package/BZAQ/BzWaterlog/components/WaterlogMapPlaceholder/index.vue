...
const tiles = computed(() => {
  ...
  for (let x = tileConfig.minX; x <= tileConfig.maxX; x += 1) {
    for (let y = tileConfig.minY; y <= tileConfig.maxY; y += 1) {
      list.push({
        key: `${x}-${y}`,
        baseSrc: require(`./img/output-yuzhong-z17-bbox/chongqing-yuzhong-z17-bbox-z17-17.lcNAnifvR/${tileConfig.z}/${x}/${y}.jpg`),
        labelSrc: require(`./img/output-yuzhong-z17-label-bbox/chongqing-yuzhong-z17-label-bbox-z17-17.N2Fp4ifDg/${tileConfig.z}/${x}/${y}.png`),
        style: {
