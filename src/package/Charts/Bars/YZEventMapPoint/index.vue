...
function addYZZBPoints(stations: YZZBInterface[]) {
  stations.forEach(station => {
    ...
    const entity = window.viewer.entities.add({
      ...
      billboard: {
        image: '/api/api/download/2062465487637368834',
...
function YZZBPointClickEvent() {
  ...
  pointClickHandler.setInputAction(async movement => {
    ...
    if (type === 'stationPoint') {
      ...
      await emitEventDetail(station.eventNo)
      // axios
...
const emitEventDetail = async (eventNo: string) => {
  if (!eventNo) return

  try {
    const response = await axios.post(EVENT_DETAIL_API, {
      regionCode: ['500103'],
    ...
    props.bus?.emit('EventDetailModal:eventData', {
      data: {
        flowId: detailItem.flowId || '',
        eventNum: eventNo
      },
      source: sourceName
    })
  } catch (error) {
    console.error('YZEventMapPoint fetch event detail failed:', error)
  }
}
...
const sourceName = 'YZEventMapPoint'
...
const loadYZZBData = () => {
  axios.get(`http://23.99.16.86:11001/api/v1/system/map/mapList/eventList`).then(res => {
    if (res.data.code !== '000000') return
