// HighCharts 三维饼图切换脚本
export const initAnimate = HighCharts => {
  (function(HighCharts) {
    HighCharts.wrap(HighCharts.seriesTypes.pie.prototype, 'translate', function(proceed) {
      ...
      this.data.forEach(d => {
        ...
        if (d.options.depth && typeof d.options.depth === 'number') {
          d.shapeArgs.depth = d.options.depth
    ...
    HighCharts.SVGRenderer.prototype.arc3dPath = function(shapeArgs) {
      ...
      const start = shapeArgs.start || 0 // start angle
      const end = (shapeArgs.end || 0) - 0.00001 // end angle
      const r = shapeArgs.r || 0 // radius
      const ir = shapeArgs.innerR || 0 // inner radius
      const d = shapeArgs.depth || 0 // depth
      const alpha = shapeArgs.alpha || 0 // alpha rotation of the chart
      const beta = shapeArgs.beta || 0 // beta rotation of the chart
      // Derived Variables
